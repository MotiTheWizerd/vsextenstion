...
export function registerCommands(context: vscode.ExtensionContext) {
  console.log("[RayDaemon] registerCommands() in src/commands/index.ts executed.");
    ...
    vscode.commands.registerCommand('raydaemon.openWelcomeView', async () => {
      try {
        ...
        try {
          ...
          console.log('[RayDaemon] openView command not available, using focus instead');
          await vscode.commands.executeCommand('workbench.view.extension.rayDaemonContainer');
        }
      } catch (error) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("raydaemon.openChatPanel", () => {
              ...
              if (message.filePath) {
                try {
                  ...
                  try {
                    ...
                    vscode.window.showErrorMessage(`Failed to show diff or open file: ${message.filePath}`);
                  }
                }
              }
              break;
      ...
      panel.webview.html = getWebviewContent(panel.webview, context, webviewConfig);
    })
  );
